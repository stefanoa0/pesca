...
////////////////BIOMETRIA PEIXES //////////////////////////////////////////////////////////////////////
    ...
    public function cpue($where = null){
        $dbTable = new Application_Model_DbTable_VEntrevistaJerere();
        $select = $dbTable->select()->setIntegrityCheck(false)->
                from('v_entrevista_jerere', "(cast(date_part('month'::text, fd_data) as varchar)) || '/' || (cast(date_part('year'::text, fd_data) as varchar)) as mesAno")->
                joinLeft('v_jerere_has_t_especie_capturada', 'v_entrevista_jerere.jre_id = v_jerere_has_t_especie_capturada.jre_id'
                , array('v_jerere_has_t_especie_capturada.jre_id','sum(v_jerere_has_t_especie_capturada.spc_peso_kg) as cpue', 'v_entrevista_jerere.tl_local','v_entrevista_jerere.pto_nome'))->
                group(array('v_jerere_has_t_especie_capturada.jre_id', "(cast(date_part('month'::text, fd_data) as varchar)) || '/' || (cast(date_part('year'::text, fd_data) as varchar))",'v_entrevista_jerere.tl_local','v_entrevista_jerere.pto_nome'))->
