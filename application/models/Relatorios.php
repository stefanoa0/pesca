...
class Application_Model_Relatorios {
    ...
    public function selectNomeEspecies($where = null, $limit = null) {
        $this->dbTableArrastoHasEspCapturada = new Application_Model_DbTable_VArrastoFundoHasEspecieCapturada();

        $select = $this->dbTableArrastoHasEspCapturada->select()->setIntegrityCheck(false)->
                from('v_arrastofundo_has_t_especie_capturada', array('esp_nome_comum'))->distinct(true)->
                joinLeft('v_entrevista_arrasto', 'v_arrastofundo_has_t_especie_capturada.af_id = v_entrevista_arrasto.af_id', array('esp_nome_comum' => 'v_arrastofundo_has_t_especie_capturada.esp_nome_comum'))
    ...
    public function selectCapturaByArteArrasto($where = null) {
        ...
        $select = $dbTable->select()->setIntegrityCheck(false)->
                ...
                joinLeft('v_arrastofundo_has_t_especie_capturada', 'v_entrevista_arrasto.af_id = v_arrastofundo_has_t_especie_capturada.af_id', array('quant' => new Zend_Db_Expr('sum(v_arrastofundo_has_t_especie_capturada.spc_quantidade)'), 'peso' => new Zend_Db_Expr('sum(CASE When v_arrastofundo_has_t_especie_capturada.esp_id = 4532 Or v_arrastofundo_has_t_especie_capturada.esp_id = 4533 Or v_arrastofundo_has_t_especie_capturada.esp_id = 4534 Or v_arrastofundo_has_t_especie_capturada.esp_id = 4536 THEN v_arrastofundo_has_t_especie_capturada.spc_peso_kg Else 0 END )')))->
    ...
    public function selectNomeEspeciesCalao($where = null, $limit = null) {
        $this->dbTableCalaoHasEspCapturada = new Application_Model_DbTable_VCalaoHasEspecieCapturada();
        $select = $this->dbTableCalaoHasEspCapturada->select()->setIntegrityCheck(false)->from('v_calao_has_t_especie_capturada', array('esp_nome_comum'))->distinct(true)->joinLeft('v_entrevista_calao', 'v_calao_has_t_especie_capturada.cal_id = v_entrevista_calao.cal_id', array('esp_nome_comum' => 'v_calao_has_t_especie_capturada.esp_nome_comum'))->order('esp_nome_comum')->limit($limit);
        if (!is_null($where)) {
            $select->where($where);
        }
    ...
    public function selectCapturaByArteCalao($where = null) {
        ...
        $select = $dbTable->select()->setIntegrityCheck(false)->
                from('v_entrevista_calao', 'v_entrevista_calao.cal_id')->joinLeft('v_calao_has_t_especie_capturada', 'v_entrevista_calao.cal_id = v_calao_has_t_especie_capturada.cal_id', array('quant' => new Zend_Db_Expr('sum(v_calao_has_t_especie_capturada.spc_quantidade)'), 'peso' => new Zend_Db_Expr('sum(v_calao_has_t_especie_capturada.spc_peso_kg)')))->
    ...
    public function selectNomeEspeciesColetaManual($where = null, $limit = null) {
        $this->dbTableColetaManualHasEspCapturada = new Application_Model_DbTable_VColetaManualHasEspecieCapturada();
        $select = $this->dbTableColetaManualHasEspCapturada->select()->setIntegrityCheck(false)->from('v_coletamanual_has_t_especie_capturada', array('esp_nome_comum'))->distinct(true)->joinLeft('v_entrevista_coletamanual', 'v_coletamanual_has_t_especie_capturada.cml_id = v_entrevista_coletamanual.cml_id', array('esp_nome_comum' => 'v_coletamanual_has_t_especie_capturada.esp_nome_comum'))->order('esp_nome_comum')->limit($limit);
        if (!is_null($where)) {
            $select->where($where);
        }
    ...
    public function selectCapturaByArteColeta($where = null) {
        ...
        $select = $dbTable->select()->setIntegrityCheck(false)->
                from('v_entrevista_coletamanual', 'v_entrevista_coletamanual.cml_id')->joinLeft('v_coletamanual_has_t_especie_capturada', 'v_entrevista_coletamanual.cml_id = v_coletamanual_has_t_especie_capturada.cml_id', array('quant' => new Zend_Db_Expr('sum(v_coletamanual_has_t_especie_capturada.spc_quantidade)'), 'peso' => new Zend_Db_Expr('sum(v_coletamanual_has_t_especie_capturada.spc_peso_kg)')))->
    ...
    public function selectNomeEspeciesEmalhe($where = null, $limit = null) {
        $this->dbTableEmalheHasEspCapturada = new Application_Model_DbTable_VEmalheHasEspecieCapturada();
        $select = $this->dbTableEmalheHasEspCapturada->select()->setIntegrityCheck(false)->from('v_emalhe_has_t_especie_capturada', array('esp_nome_comum'))->distinct(true)->joinLeft('v_entrevista_emalhe', 'v_emalhe_has_t_especie_capturada.em_id = v_entrevista_emalhe.em_id', array('esp_nome_comum' => 'v_emalhe_has_t_especie_capturada.esp_nome_comum'))->order('esp_nome_comum')->limit($limit);
        if (!is_null($where)) {
            $select->where($where);
        }
    ...
    public function selectNomeEspeciesGrosseira($where = null, $limit = null) {
        $this->dbTableGrosseiraHasEspCapturada = new Application_Model_DbTable_VGrosseiraHasEspecieCapturada();
        $select = $this->dbTableGrosseiraHasEspCapturada->select()->setIntegrityCheck(false)->from('v_grosseira_has_t_especie_capturada', array('esp_nome_comum'))->distinct(true)->joinLeft('v_entrevista_grosseira', 'v_grosseira_has_t_especie_capturada.grs_id = v_entrevista_grosseira.grs_id', array('esp_nome_comum' => 'v_grosseira_has_t_especie_capturada.esp_nome_comum'))->order('esp_nome_comum')->limit($limit);
        if (!is_null($where)) {
            $select->where($where);
        }
    ...
    public function selectCapturaByArteGrosseira($where = null) {
        ...
        $select = $dbTable->select()->setIntegrityCheck(false)->
                from('v_entrevista_grosseira', 'v_entrevista_grosseira.grs_id')->joinLeft('v_grosseira_has_t_especie_capturada', 'v_entrevista_grosseira.grs_id = v_grosseira_has_t_especie_capturada.grs_id', array('quant' => new Zend_Db_Expr('sum(v_grosseira_has_t_especie_capturada.spc_quantidade)'), 'peso' => new Zend_Db_Expr('sum(v_grosseira_has_t_especie_capturada.spc_peso_kg)')))->
    ...
    public function selectNomeEspeciesJerere($where = null, $limit = null) {
        $this->dbTableJerereHasEspCapturada = new Application_Model_DbTable_VJerereHasEspecieCapturada();
        $select = $this->dbTableJerereHasEspCapturada->select()->setIntegrityCheck(false)->from('v_jerere_has_t_especie_capturada', array('esp_nome_comum'))->distinct(true)->joinLeft('v_entrevista_jerere', 'v_jerere_has_t_especie_capturada.jre_id = v_entrevista_jerere.jre_id', array('esp_nome_comum' => 'v_jerere_has_t_especie_capturada.esp_nome_comum'))->order('esp_nome_comum')->limit($limit);
        if (!is_null($where)) {
            $select->where($where);
        }
    ...
    public function selectCapturaByArteJerere($where = null) {
        ...
        $select = $dbTable->select()->setIntegrityCheck(false)->
                ...
                joinLeft('v_jerere_has_t_especie_capturada', 'v_entrevista_jerere.jre_id = v_jerere_has_t_especie_capturada.jre_id', array('quant' => new Zend_Db_Expr('sum(v_jerere_has_t_especie_capturada.spc_quantidade)'), 'peso' => new Zend_Db_Expr('sum( CASE When v_jerere_has_t_especie_capturada.esp_id = 4596 THEN v_jerere_has_t_especie_capturada.spc_peso_kg Else 0 END)')))->
    ...
    public function selectNomeEspeciesLinha($where = null, $limit = null) {
        $this->dbTableLinhaHasEspCapturada = new Application_Model_DbTable_VLinhaHasEspecieCapturada();
        $select = $this->dbTableLinhaHasEspCapturada->select()->setIntegrityCheck(false)->from('v_linha_has_t_especie_capturada', array('esp_nome_comum'))->distinct(true)->joinLeft('v_entrevista_linha', 'v_linha_has_t_especie_capturada.lin_id = v_entrevista_linha.lin_id', array('esp_nome_comum' => 'v_linha_has_t_especie_capturada.esp_nome_comum'))->order('esp_nome_comum')->limit($limit);
        if (!is_null($where)) {
            $select->where($where);
        }
    ...
    public function selectCapturaByArteLinha($where = null) {
        ...
        $select = $dbTable->select()->setIntegrityCheck(false)->
                from('v_entrevista_linha', 'v_entrevista_linha.lin_id')->joinLeft('v_linha_has_t_especie_capturada', 'v_entrevista_linha.lin_id = v_linha_has_t_especie_capturada.lin_id', array('quant' => new Zend_Db_Expr('sum(v_linha_has_t_especie_capturada.spc_quantidade)'), 'peso' => new Zend_Db_Expr('sum(v_linha_has_t_especie_capturada.spc_peso_kg)')))->
    ...
    public function selectNomeEspeciesLinhaFundo($where = null, $limit = null) {
        $this->dbTableLinhaFundoHasEspCapturada = new Application_Model_DbTable_VLinhaFundoHasEspecieCapturada();
        $select = $this->dbTableLinhaFundoHasEspCapturada->select()->setIntegrityCheck(false)->from('v_linhafundo_has_t_especie_capturada', array('esp_nome_comum'))->distinct(true)->joinLeft('v_entrevista_linhafundo', 'v_linhafundo_has_t_especie_capturada.lf_id = v_entrevista_linhafundo.lf_id', array('esp_nome_comum' => 'v_linhafundo_has_t_especie_capturada.esp_nome_comum'))->order('esp_nome_comum')->limit($limit);
        if (!is_null($where)) {
            $select->where($where);
        }
    ...
    public function selectCapturaByArteLinhaFundo($where = null) {
        ...
        $select = $dbTable->select()->setIntegrityCheck(false)->
                from('v_entrevista_linhafundo', 'v_entrevista_linhafundo.lf_id')->joinLeft('v_linhafundo_has_t_especie_capturada', 'v_entrevista_linhafundo.lf_id = v_linhafundo_has_t_especie_capturada.lf_id', array('quant' => new Zend_Db_Expr('sum(v_linhafundo_has_t_especie_capturada.spc_quantidade)'), 'peso' => new Zend_Db_Expr('sum(v_linhafundo_has_t_especie_capturada.spc_peso_kg)')))->
    ...
    public function selectNomeEspeciesManzua($where = null, $limit = null) {
        $this->dbTableManzuaHasEspCapturada = new Application_Model_DbTable_VManzuaHasEspecieCapturada();
        $select = $this->dbTableManzuaHasEspCapturada->select()->setIntegrityCheck(false)->from('v_manzua_has_t_especie_capturada', array('esp_nome_comum'))->distinct(true)->joinLeft('v_entrevista_manzua', 'v_manzua_has_t_especie_capturada.man_id = v_entrevista_manzua.man_id', array('esp_nome_comum' => 'v_manzua_has_t_especie_capturada.esp_nome_comum'))->order('esp_nome_comum')->limit($limit);
        if (!is_null($where)) {
            $select->where($where);
        }
    ...
    public function selectCapturaByArteManzua($where = null) {
        ...
        $select = $dbTable->select()->setIntegrityCheck(false)->
                ...
                joinLeft('v_manzua_has_t_especie_capturada', 'v_entrevista_manzua.man_id = v_manzua_has_t_especie_capturada.man_id', array('quant' => new Zend_Db_Expr('sum(v_manzua_has_t_especie_capturada.spc_quantidade)'), 'peso' => new Zend_Db_Expr('sum(CASE When v_manzua_has_t_especie_capturada.esp_id = 4596 THEN v_manzua_has_t_especie_capturada.spc_peso_kg Else 0 END)')))->
...
/////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function selectNomeEspeciesMergulho($where = null, $limit = null) {
        $this->dbTableMergulhoHasEspCapturada = new Application_Model_DbTable_VMergulhoHasEspecieCapturada();
        $select = $this->dbTableMergulhoHasEspCapturada->select()->setIntegrityCheck(false)->from('v_mergulho_has_t_especie_capturada', array('esp_nome_comum'))->distinct(true)->joinLeft('v_entrevista_mergulho', 'v_mergulho_has_t_especie_capturada.mer_id = v_entrevista_mergulho.mer_id', array('esp_nome_comum' => 'v_mergulho_has_t_especie_capturada.esp_nome_comum'))->order('esp_nome_comum')->limit($limit);
        if (!is_null($where)) {
            $select->where($where);
        }
    ...
    public function selectCapturaByArteMergulho($where = null) {
        ...
        $select = $dbTable->select()->setIntegrityCheck(false)->
                ...
                ->joinLeft('v_mergulho_has_t_especie_capturada', 'v_entrevista_mergulho.mer_id = v_mergulho_has_t_especie_capturada.mer_id', array('quant' => new Zend_Db_Expr('sum(v_mergulho_has_t_especie_capturada.spc_quantidade)'), 'peso' => new Zend_Db_Expr('sum(v_mergulho_has_t_especie_capturada.spc_peso_kg)')))->
...
/////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function selectNomeEspeciesRatoeira($where = null, $limit = null) {
        $this->dbTableRatoeiraHasEspCapturada = new Application_Model_DbTable_VRatoeiraHasEspecieCapturada();
        $select = $this->dbTableRatoeiraHasEspCapturada->select()->setIntegrityCheck(false)->from('v_ratoeira_has_t_especie_capturada', array('esp_nome_comum'))->distinct(true)->joinLeft('v_entrevista_ratoeira', 'v_ratoeira_has_t_especie_capturada.rat_id = v_entrevista_ratoeira.rat_id', array('esp_nome_comum' => 'v_ratoeira_has_t_especie_capturada.esp_nome_comum'))->order('esp_nome_comum')->limit($limit);
        if (!is_null($where)) {
            $select->where($where);
        }
    ...
    public function selectCapturaByArteRatoeira($where = null) {
        ...
        $select = $dbTable->select()->setIntegrityCheck(false)->
                ...
                joinLeft('v_ratoeira_has_t_especie_capturada', 'v_entrevista_ratoeira.rat_id = v_ratoeira_has_t_especie_capturada.rat_id', array('quant' => new Zend_Db_Expr('sum(v_ratoeira_has_t_especie_capturada.spc_quantidade)'), 'peso' => new Zend_Db_Expr('sum(CASE When v_ratoeira_has_t_especie_capturada.esp_id = 4580 THEN v_ratoeira_has_t_especie_capturada.spc_peso_kg Else 0 END)')))->
...
/////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function selectNomeEspeciesSiripoia($where = null, $limit = null) {
        $this->dbTableSiripoiaHasEspCapturada = new Application_Model_DbTable_VSiripoiaHasEspecieCapturada();
        $select = $this->dbTableSiripoiaHasEspCapturada->select()->setIntegrityCheck(false)->from('v_siripoia_has_t_especie_capturada', array('esp_nome_comum'))->distinct(true)->joinLeft('v_entrevista_siripoia', 'v_siripoia_has_t_especie_capturada.sir_id = v_entrevista_siripoia.sir_id', array('esp_nome_comum' => 'v_siripoia_has_t_especie_capturada.esp_nome_comum'))->order('esp_nome_comum')->limit($limit);
        if (!is_null($where)) {
            $select->where($where);
        }
    ...
    public function selectCapturaByArteSiripoia($where = null) {
        ...
        $select = $dbTable->select()->setIntegrityCheck(false)->
                from('v_entrevista_siripoia', 'v_entrevista_siripoia.sir_id')->joinLeft('v_siripoia_has_t_especie_capturada', 'v_entrevista_siripoia.sir_id = v_siripoia_has_t_especie_capturada.sir_id', array('quant' => new Zend_Db_Expr('sum(v_siripoia_has_t_especie_capturada.spc_quantidade)'), 'peso' => new Zend_Db_Expr('sum(v_siripoia_has_t_especie_capturada.spc_peso_kg)')))->
...
/////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function selectNomeEspeciesTarrafa($where = null, $limit = null) {
        $this->dbTableTarrafaHasEspCapturada = new Application_Model_DbTable_VTarrafaHasEspecieCapturada();
        $select = $this->dbTableTarrafaHasEspCapturada->select()->setIntegrityCheck(false)->from('v_tarrafa_has_t_especie_capturada', array('esp_nome_comum'))->distinct(true)->joinLeft('v_entrevista_tarrafa', 'v_tarrafa_has_t_especie_capturada.tar_id = v_entrevista_tarrafa.tar_id', array('esp_nome_comum' => 'v_tarrafa_has_t_especie_capturada.esp_nome_comum'))->order('esp_nome_comum')->limit($limit);
        if (!is_null($where)) {
            $select->where($where);
        }
    ...
    public function selectCapturaByArteTarrafa($where = null) {
        ...
        $select = $dbTable->select()->setIntegrityCheck(false)->
                from('v_entrevista_tarrafa', 'v_entrevista_tarrafa.tar_id')->joinLeft('v_tarrafa_has_t_especie_capturada', 'v_entrevista_tarrafa.tar_id = v_tarrafa_has_t_especie_capturada.tar_id', array('quant' => new Zend_Db_Expr('sum(v_tarrafa_has_t_especie_capturada.spc_quantidade)'), 'peso' => new Zend_Db_Expr('sum(v_tarrafa_has_t_especie_capturada.spc_peso_kg)')))->
...
/////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function selectNomeEspeciesVaraPesca($where = null, $limit = null) {
        $this->dbTableVaraPescaHasEspCapturada = new Application_Model_DbTable_VVaraPescaHasEspecieCapturada();
        $select = $this->dbTableVaraPescaHasEspCapturada->select()->setIntegrityCheck(false)->from('v_varapesca_has_t_especie_capturada', array('esp_nome_comum'))->distinct(true)->joinLeft('v_entrevista_varapesca', 'v_varapesca_has_t_especie_capturada.vp_id = v_entrevista_varapesca.vp_id', array('esp_nome_comum' => 'v_varapesca_has_t_especie_capturada.esp_nome_comum'))->order('esp_nome_comum')->limit($limit);
        if (!is_null($where)) {
            $select->where($where);
        }
    ...
    public function selectCapturaByArteVaraPesca($where = null) {
        ...
        $select = $dbTable->select()->setIntegrityCheck(false)->
                from('v_entrevista_varapesca', 'v_entrevista_varapesca.vp_id')->joinLeft('v_varapesca_has_t_especie_capturada', 'v_entrevista_varapesca.vp_id = v_varapesca_has_t_especie_capturada.vp_id', array('quant' => new Zend_Db_Expr('sum(v_varapesca_has_t_especie_capturada.spc_quantidade)'), 'peso' => new Zend_Db_Expr('sum(v_varapesca_has_t_especie_capturada.spc_peso_kg)')))->
